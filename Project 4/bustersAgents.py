...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        mostLikelyPos = util.Counter()
        for ghost in livingGhosts:
            mostLikelyPos[ghost-1] = livingGhostPositionDistributions[ghost-1].argMax()

        closestPos = None
        closestVal = None
        for pos in mostLikelyPos.values():
            testDist = self.distancer.getDistance(pacmanPosition, pos)
            if closestVal is None or closestVal > testDist:
                closestVal = testDist
                closestPos = pos
        ...
        for action in legal:
            ...
            sucDist = self.distancer.getDistance(closestPos, sucPos)
            if bestVal is None or bestVal > sucDist:
                bestVal = sucDist
                bestAction = action
